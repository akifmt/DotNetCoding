namespace BlazorAppRadzenSearch.Data;

public class BlogPostService
{
    public Task<IEnumerable<BlogPost>> GetBlogPostsAsync()
    {
        return Task.FromResult(GetAllBlogPosts());
    }

    public Task<(IEnumerable<BlogPost> Result, int TotalCount)> GetBlogPostsbyPageAsync(int? top, int? skip, bool count = true,
                                                                       string? searchString = null, Func<BlogPost, string>? searchProperty = null)
    {
        var result = GetAllBlogPosts().AsQueryable();

        if (searchProperty != null && searchString != null)
        {
            //Func<BlogPost, bool> filter = x => searchProperty(x).ToLower().Contains(searchString.ToLower());
            bool filter(BlogPost x) => searchProperty(x).ToLower().Contains(searchString.ToLower());
            result = result.Where(filter).AsQueryable();
        }

        // get all items
        if (skip == null || top == null)
            return Task.FromResult((result.AsEnumerable(), count ? result.Count() : 0));

        // get paged items
        return Task.FromResult((result.Skip(skip.Value).Take(top.Value).AsEnumerable(), count ? result.Count() : 0));
    }
